...
const GUNNER_UPGRADE_MAP: Record<string, Partial<CardDef>> = {
  ...
  g_reload:         { name: '긴급 회피+',      description: '방어도 +8.',                       effects: [{ kind: 'block', amount: 8 }] },
  g_headshot:       { name: '산탄+',           description: '10 데미지. 취약 +3.',              effects: [{ kind: 'damage', amount: 10 }, { kind: 'apply_enemy', status: 'vulnerable', amount: 3 }] },
  g_double_gunhawk: { name: '이중 사격+',    description: '7 데미지를 두 번.',                effects: [{ kind: 'damage', amount: 7, times: 2 }] },
  g_western_fire:   { name: '난사+',        description: '모든 적에게 11 데미지.',           effects: [{ kind: 'damage_all', amount: 11 }] },
  g_frag:           { name: '섬광탄+',      description: '모든 적에게 7 데미지. 취약 +1.',   effects: [{ kind: 'damage_all', amount: 7 }, { kind: 'apply_all', status: 'vulnerable', amount: 1 }] },
  ...
  g_smoke:          { name: '연막 사격+', description: '모든 적에게 약화 +3.',            effects: [{ kind: 'apply_all', status: 'weak', amount: 3 }] },
  g_booster:        { name: '전투 집중+',      description: '힘 +4.',                           effects: [{ kind: 'apply_self', status: 'strength', amount: 4 }] },
  g_reposit:        { name: '엄폐+',           description: '방어도 +11. 1장 드로우.',          effects: [{ kind: 'block', amount: 11 }, { kind: 'draw', amount: 1 }] },
  g_ammo:           { name: '조준 사격+',        description: '2장 드로우. 무작위 카드 1장 소멸.',effects: [{ kind: 'draw', amount: 2 }, { kind: 'exhaust_random_hand' }] },
  g_armor:          { name: '냉각탄+',           description: '7 데미지. 빙결 +2. 소멸.',              effects: [{ kind: 'damage', amount: 7 }, { kind: 'apply_enemy', status: 'freeze', amount: 2 }] },
  ...
  g_sparrow:        { name: '장갑 코팅+',         description: '턴 종료 시 방어도 +4.',                effects: [{ kind: 'apply_self', status: 'metallicize', amount: 4 }] },
  g_gods_hail:      { name: '강철의 비+',               description: '8 데미지. 사용할 때마다 영구 데미지 +3.', effects: [{ kind: 'damage', amount: 8 }], scaling: { kind: 'on_play', amount: 3 } },
  g_divergent:      { name: '탄약 회수+',            description: '카드 소멸 시 카드 2장 드로우.',         effects: [{ kind: 'apply_self', status: 'on_exhaust_draw', amount: 2 }] },
  ...
  g_arms_dealer:    { name: '결사 사격+',       description: 'HP -3. 30 데미지.',                    effects: [{ kind: 'lose_hp', amount: 3 }, { kind: 'damage', amount: 30 }] },
  g_airstrike:      { name: '폭풍 사격+',   description: '모든 적에게 24 데미지.',               effects: [{ kind: 'damage_all', amount: 24 }] },
  // 콤보 시너지
  ...
  g_helios:         { name: '최종 병기+',      description: '45 데미지. 모든 적 약화 +3. 소멸.',   effects: [{ kind: 'damage', amount: 45 }, { kind: 'apply_all', status: 'weak', amount: 3 }] },
