...
export function renderHelp(): HTMLElement {
  ...
  helpTitle.innerHTML = `${ic('question')} 도움말`;
  ...
  wrapper.appendChild(section(`${ic('card')} 카드 사용법`, [
    '카드는 화면 아래쪽 "핸드"에 보입니다. 좌상단의 노란 숫자가 비용(에너지).',
    `카드 종류는 3가지: ${ic('sword')} 공격 / ${ic('shield')} 방어 / ${ic('sparkle')} 효과.`,
    `${ic('poison')} 저주 카드: 이벤트에서 획득. 사용 불가, 덱만 차지. (부식: HP-2, 의심: 약화+1, 기생충: 소멸 불가)`,
    `${ic('sparkle')} 축복 카드: 이벤트 보상. 0코스트 강력 효과, 소멸.`,
    `빨간 자물쇠(${ic('lock')})가 뜨면 에너지 부족 — 못 씀.`,
    '카드에 적힌 키워드:',
  ...
  wrapper.appendChild(section(`${ic('star')} 카드 강화 (모닥불 대장간)`, [
  ...
  wrapper.appendChild(section(`${ic('star')} 카드 타게팅 — 중요!`, [
  ...
  wrapper.appendChild(section(`${ic('lightning')} 에너지 &amp; 턴`, [
  ...
  wrapper.appendChild(section(`${ic('shield')} 방어도 &amp; 적 의도`, [
    '적 머리 위 표시가 "다음 턴 행동 예고":',
    `  ${ic('sword')} 숫자 = 공격 (수치는 취약/약화/힘 적용된 실제 데미지)`,
    `  ${ic('shield')} = 방어 / ↑ = 자기 강화 / ↓ = 디버프`,
    `내 방어도(${ic('shield')})로 다 막히면 적 의도가 푸르게 표시됨 (안전!).`,
    '방어도는 다음 턴 시작 시 사라짐 — 매 턴 새로 쌓아야 함.',
  ]));

  wrapper.appendChild(section(`${ic('sparkle')} 상태이상`, [
  ...
  wrapper.appendChild(section(`${ic('star')} 맵 노드`, [
    `${ic('sword')} 전투 — 일반 몬스터, 골드+카드 보상`,
    `${ic('skull')} 엘리트 — 강한 적, 희귀 유물 보상. 챕터당 첫 엘리트에서 ${ic('key')} 키 드롭`,
    `${ic('fire')} 모닥불 — 휴식(HP)/대장간(강화)/정화(제거)/복제 중 1택`,
    `${ic('gold')} 보물 — 무료 골드+카드 선택`,
    `${ic('cart')} 상점 — 카드·유물 구매, 카드 제거`,
    `${ic('question')} 이벤트 — 선택지에 따라 다른 결과`,
    `${ic('crown')} 보스 — 챕터 보스. 처치 시 보스 유물 1개 선택`,
  ]));

  wrapper.appendChild(section(`${ic('key')} 진엔딩 (키 시스템)`, [
  ...
  wrapper.appendChild(section(`${ic('lightning')} 핫키 (PC)`, [
    '1~9 — 핸드 N번째 카드 선택/사용',
  ...
  wrapper.appendChild(section(`${ic('potion_small')} 포션`, [
    '전투 승리 시 30~50% 확률로 드롭 (최대 3개 보유).',
    '전투 중 좌측 포션 슬롯 탭으로 즉시 사용.',
    '일부 포션(독/화염)은 적을 지정해야 함.',
  ]));

  wrapper.appendChild(section(`${ic('gem')} 유물`, [
    '영구 효과를 주는 아이템. 한 번 얻으면 런 끝까지 발동.',
    '엘리트/상점/이벤트/보상에서 획득.',
    '엘리트 유물: 엘리트 전투에서만 드롭. 클래스 전용 유물 포함.',
    `${ic('lightning')} 시너지: 특정 유물 2개를 함께 보유하면 추가 효과 발동! (금색 테두리)`,
    '보스 유물(노란색)은 챕터 클리어 시 3개 중 1개 선택 — 매우 강력.',
  ]));

  wrapper.appendChild(section(`${ic('card')} 도감 보상`, [
  ...
  wrapper.appendChild(section(`${ic('medal')} 업적 보상`, [
    '업적을 달성할수록 칭호와 시작 보너스를 얻습니다.',
    `3개 — ${ic('star')} 초보 모험가 (시작 골드 +20)`,
    `8개 — ${ic('sword')} 숙련된 전사 (시작 최대HP +3)`,
    `15개 — ${ic('fire')} 던전 정복자 (시작 골드 +30)`,
    `22개 — ${ic('crown')} 전설의 영웅`,
    '칭호는 리더보드에 표시됩니다.',
  ]));
