...
export function renderHelp(): HTMLElement {
  ...
  wrapper.appendChild(section('🎯 카드 타게팅 — 중요!', [
    ...
    ['  ① 카드를 한 번 탭/클릭 → 카드가 위로 떠오름 (선택됨)', { highlight: true }],
    ['  ② 그 다음 공격할 적을 탭/클릭 → 발동', { highlight: true }],
...
function section(title: string, lines: (string | [string, { highlight?: boolean }])[]): HTMLElement {
  ...
  for (const item of lines) {
    if (typeof item === 'string') {
      ...
      const cls = 'help-line' + (item[1].highlight ? ' highlight' : '');
      body.appendChild(el('div', { class: cls }, item[0]));
    }
  }
